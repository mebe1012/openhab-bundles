...
public class PhilipsTvHandler extends BaseThingHandler implements DiscoveryListener {
  ...
  public void initialize() {
    ...
    if ((config.pairingCode == null) && (config.username == null) && (config.password == null)) {
      ...
      try {
        initPairingCodeRetrieval(); //TODO wirft keine Exception wenn URL auf Grund anderer Version nicht gefunden wird
...
//  private void retrieveMacAddressAndUpdateThing() {
//    try {
//      String macFromEnabledInterface = new NetworkDetailsService().getMacFromEnabledInterface(client, target);
//      if (macFromEnabledInterface.isEmpty()) {
//        logger.info("Mac address for Wake-On-LAN could not be retrieved, it seems that its not yet enabled via Settings on the TV.");
//      } else {
//        Configuration thingConfig = getConfig();
//        thingConfig.put("mac", macFromEnabledInterface);
//        updateConfiguration(thingConfig);
//        config.mac = macFromEnabledInterface;
//      }
//    } catch (Exception e) {
//      logger.error("Error during retrieval and setting of mac address: {}", e.getMessage(), e);
//    }
//  }
